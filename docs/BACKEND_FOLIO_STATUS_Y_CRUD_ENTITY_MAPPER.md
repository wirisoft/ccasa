# Backend: `FolioStatusEnum` y `CrudEntityMapper` (relaciones CRUD)

Documento de referencia para el estado de folio y el mapeo de `CrudRequestDTO.values` hacia entidades JPA. Complementa [BACKEND_FOLIO_ENTIDADES_Y_CRUD.md](./BACKEND_FOLIO_ENTIDADES_Y_CRUD.md).

---

## `FolioStatusEnum`

Ruta: `ccasaBackend/src/main/java/com/backend/ccasa/service/models/enums/FolioStatusEnum.java`

Valores serializados en JSON como strings (`"Open"`, `"Closed"`), coherentes con `@Enumerated(EnumType.STRING)` en `FolioEntity`.
...
## Cómo `CrudEntityMapper` trata las relaciones `@ManyToOne` (y el mapa `values`)

No hay introspección genérica sobre anotaciones JPA. Cada tipo de entidad tiene un bloque **manual** en `apply(Class<E> entityClass, E entity, Map<String, Object> values, EntityManager entityManager)` que:

1. Comprueba claves concretas del mapa con `values.containsKey("...")`.
2. Para FKs, obtiene el valor crudo (`values.get("xxxId")`) y llama a `requireActive(entityManager, ClaseRelacionada.class, rawId)`.
3. `requireActive` convierte el id con `CrudValueHelper.asLong(rawId)`, ejecuta `entityManager.find(...)`, y si la entidad no existe o tiene borrado lógico (`deletedAt != null`) lanza `IllegalArgumentException`.

### Ejemplo: `FolioEntity` en `apply`

Claves reconocidas para folio (entrada desde el frontend en `CrudRequestDTO.values`):

| Clave en `values` | Efecto |
|-------------------|--------|
| `folioNumber` | Número de folio |
| `status` | `FolioStatusEnum` (p. ej. `"Open"`, `"Closed"`) |
| `folioBlockId` | Carga `FolioBlockEntity` y `setFolioBlock(...)` |
| `logbookId` | Carga `LogbookEntity` y `setLogbook(...)` |

- **`{ "logbookId": 5 }`**: sí resuelve la FK al bitácora con id 5.
- **`{ "logbook": 5 }`**: **no** está implementado; esa clave no se lee en el mapper, así que **no** asigna el logbook.

No se usa el nombre del campo Java de la entidad (`logbook`, `folioBlock`), sino las claves acordadas en código (`logbookId`, `folioBlockId`). El mismo patrón se repite en otras entidades (`roleId`, `folioId`, `reagentId`, etc.).

### Serialización de vuelta: `toValues` para `FolioEntity`

En respuestas CRUD, las relaciones se exponen como ids:

- `folioBlockId`
- `logbookId`
- más `folioNumber`, `status` y campos de auditoría vía `putAudit`.

---

## `CrudEntityMapper.java` (archivo completo)

Ruta: `ccasaBackend/src/main/java/com/backend/ccasa/service/impl/support/CrudEntityMapper.java`
...
> **Nota:** Si el código fuente en el repositorio cambia, la fuente de verdad sigue siendo el archivo `.java` en `ccasaBackend`; conviene contrastar con él antes de depurar integraciones.
