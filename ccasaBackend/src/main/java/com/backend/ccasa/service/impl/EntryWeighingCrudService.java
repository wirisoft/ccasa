...
public class EntryWeighingCrudService extends AbstractEntityCrudService<EntryWeighingEntity> implements IEntryWeighingCrudService {
	...
	private void writePdf(Document document, EntryEntity entry, EntryWeighingEntity w) throws Exception {
		document.add(headerLine(entry));
		document.add(headerDividerThick());
		document.add(moduleTitleBanner("REGISTRO DE PESADA"));
		document.add(buildMainDataTable(entry, w));
		document.add(buildResultBanner(w));
	...
	private PdfPTable buildMainDataTable(EntryEntity entry, EntryWeighingEntity w) {
		PdfPTable table = new PdfPTable(new float[] { 1.1f, 1.1f, 1f, 0.9f, 1.2f });
		table.setWidthPercentage(100);
		table.setSpacingBefore(4f);
		table.addCell(dataHeaderCell("Bitácora"));
		table.addCell(dataHeaderCell("Analista"));
		table.addCell(dataHeaderCell("Reactivo pesado"));
		table.addCell(dataHeaderCell("Peso (g)"));
		table.addCell(dataHeaderCell("Solución destino"));
		Color rowBg = COLOR_WHITE;
		String logbookName = entry.getLogbook() != null ? safe(entry.getLogbook().getName()) : "";
		String reagentName = w.getReagent() != null ? safe(w.getReagent().getName()) : "";
		String solutionLine = buildSolutionLine(w.getTargetSolution());
		...
		table.addCell(dataBodyCell(reagentName, F_9_NORMAL_BLACK, rowBg, Element.ALIGN_LEFT));
		table.addCell(dataBodyCell(fixed(w.getWeightGrams(), 4), F_9_NORMAL_BLACK, rowBg, Element.ALIGN_CENTER));
		table.addCell(dataBodyCell(solutionLine, F_9_NORMAL_BLACK, rowBg, Element.ALIGN_LEFT));
		return table;
	}

	private String buildSolutionLine(SolutionEntity s) {
		if (s == null) {
			return "";
		}
		String solutionLine = safe(s.getName());
	...
	private PdfPTable buildResultBanner(EntryWeighingEntity w) {
		PdfPTable t = new PdfPTable(1);
		t.setWidthPercentage(100);
		t.setSpacingBefore(6f);
		String reagentName = w.getReagent() != null ? safe(w.getReagent().getName()) : "—";
		String line = "Peso registrado: " + fixed(w.getWeightGrams(), 4) + " g · Reactivo: " + reagentName;
